        ...
        <div class="mb-8 p-6 bg-white shadow rounded-lg">
            ...
            @foreach ($generatedFiles as $file)
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-xl font-semibold">{{ basename($file) }}</h2>
                    <!-- Download Button -->
                    <a href="{{ asset('storage/pdf/' . basename($file)) }}"
                        class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700" download>
                        Download Statement
                    </a>
                </div>
            @endforeach
